...
def _normalize_no_accents(value: str) -> str:
    value = value.lower()
...
def _ddx_normalize_spaced(value: str) -> str:
    value = _normalize_no_accents(value)
...
async def ddx_preprocess_update(bot, update) -> bool:
    ...
    try:
        ...
        logger.exception("DDX_ADMIN_CHECK_FAILED | chat_id=%s", message.chat.id)
        return False
    try:
        ...
        logger.exception("DDX_DELETE_FAILED | chat_id=%s", message.chat.id)
        return False
...
async def ddx(message: Message) -> None:
    ...
    try:
        ...
        if mode == "add":
            ...
            await message.answer(f"DDX atualizado.\nGrupo: {chat_id}\nStatus: ativo\nTotal de palavras: {len(final_words)}")
            return
        if mode == "remove":
            if len(lines) < 4:
        ...
        if mode == "test":
            ...
            await message.answer(f"DDX TESTE\nGrupo: {chat_id}\nResultado: {'detectado' if matched else 'não detectado'}")
            return
...
async def remember_join_request(request: ChatJoinRequest) -> None:
    ...
    with engine.begin() as conn:
        ...
        conn.execute(
            text("INSERT INTO join_requests (user_id, chat_id, created_at) VALUES (:user_id, :chat_id, :created_at)"),
            {"user_id": request.from_user.id, "chat_id": request.chat.id, "created_at": datetime.now(timezone.utc)},
        )
...
async def vx(message: Message) -> None:
    await _vanish_command(message, "vanish")


@router.message(Command("uv"))
async def uv(message: Message) -> None:
    await _vanish_command(message, "unvanish")


async def _vanish_command(message: Message, action: str) -> None:
    if not _is_owner_private_message(message):
        return
    lines = _lines(message)
    if len(lines) < 3:
        title = "Vanish" if action == "vanish" else "Unvanish"
        await message.answer(f"Título: {title}\n\nUse:\n/{'vx' if action == 'vanish' else 'uv'}\n<chat_id>\n<user_id>")
        return
    try:
        chat_id = _parse_chat_id(lines[1])
        user_id = _parse_user_id(lines[2])
        await _execute_action(message.bot, chat_id, user_id, action)
        _remember_group(chat_id, str(chat_id))
        await message.answer(_success_text(f"{action} executado.", f"Grupo: {chat_id}\nUsuário: {user_id}"))
    except TelegramForbiddenError:
        await message.answer(_error_text("operação não permitida", "verifique permissões do bot"))
    except Exception:
...
async def mx(message: Message) -> None:
    ...
    lines = _lines(message)
    if len(lines) < 3:
        await message.answer("Título: Mute temporário\n\nUse:\n/mx\n<chat_id>\n<user_id>\n<10m|2h|3d|i|x>")
        return
    try:
        chat_id = _parse_chat_id(lines[1])
        user_id = _parse_user_id(lines[2])
...
def _parse_xend(lines: list[str]) -> tuple[bool, int | None]:
    if len(lines) < 2:
        return False, None
    parts = lines[0].split()
    if len(parts) >= 3 and parts[1].lower() == "pin":
        return True, int(parts[2])
    if len(parts) >= 2 and parts[1].lower() == "pin" and len(lines) >= 2:
        return True, int(lines[1])
    if len(parts) >= 2:
        return False, int(parts[1])
    return False, int(lines[1])
...
async def xend(message: Message) -> None:
    ...
    if not message.reply_to_message:
        await message.answer("Use respondendo uma mensagem no privado do bot:\n/xend <chat_id>\nou\n/xend pin <chat_id>")
        return
    try:
        pin, target_chat_id = _parse_xend(_lines(message))
        if target_chat_id is None:
            raise ValueError("chat_id ausente")
        ...
        if pin:
            await message.bot.pin_chat_message(chat_id=target_chat_id, message_id=copied.message_id)
        await message.answer(_success_text("Mensagem copiada.", f"Destino: {target_chat_id}\nMessage ID: {copied.message_id}\nFixada: {'sim' if pin else 'não'}"))
    except Exception:
        logger.exception("XEND_FAILED")
        await message.answer(_error_text("falha ao copiar mensagem", "verifique chat_id, permissões do bot e se você respondeu uma mensagem"))
...
async def ximg(message: Message) -> None:
    if not _is_owner_private_message(message):
        return
    if not message.reply_to_message:
        await message.answer("Use respondendo a mídia no privado do bot:\n/ximg\n<chat_id>")
        return
    lines = _lines(message)
    if len(lines) < 2:
        await message.answer("Use:\n/ximg\n<chat_id>")
        return
    try:
        target_chat_id = _parse_chat_id(lines[1])
        copied = await message.bot.copy_message(
            chat_id=target_chat_id,
            from_chat_id=message.reply_to_message.chat.id,
            message_id=message.reply_to_message.message_id,
        )
        await message.answer(_success_text("Mídia copiada.", f"Destino: {target_chat_id}\nMessage ID: {copied.message_id}"))
    except Exception:
        logger.exception("XIMG_FAILED")
        await message.answer(_error_text("falha ao copiar mídia", "verifique chat_id, permissões e se respondeu uma mídia"))
